# /scratch////evaluation/commonsense_constraint.py
...
events = Events()
pois = pd.read_csv('../_database/public_transit_gtfs/all_poi_nearest_stops.csv')

city_state_set = open('../_database/background/citySet_with_states_140.txt','r').read().split('\n')
...
def is_valid_event(question, tested_data):
    """
    Validates that each event (name + city) exists in the sandbox CSV.
    Repetition across days is ALLOWED.
    """
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if unit.get('event') and unit['event'] != '-':
            for event in unit['event'].split(';'):
                ...
                # Extract name and city from itinerary
                name = event.rsplit(",", 1)[0].strip().lower()
                city = event.rsplit(",", 1)[-1].strip().lower()

                # 🔍 lookup event in sandbox using name + city ONLY
                res = events.data[
                    (events.data['name'].astype(str).str.strip().str.lower() == name) &
                    (events.data['city'].astype(str).str.strip().str.lower() == city)
                ]

                # ❌ If event does not exist in sandbox → INVALID
                if len(res) == 0:
                    return False, (
                        f"The event '{name}' does not exist in the sandbox "
                        f"for city '{city}' (day {i+1})."
                    )
...
def is_valid_poi_sequence(question, tested_data, plan_id=None):
    ...
    for i, unit in enumerate(tested_data[:total_days]):
        ...
        curr_acc_base_no_quote = curr_acc_base.replace("'", "").replace("’", "") if curr_acc_base else None
        prev_acc_no_quote = prev_accommodation.replace("'", "").replace("’", "") if prev_accommodation else None

        poi_entries_no_quote = [poi.replace("'", "").replace("’", "") for poi in poi_entries]
        first_poi_no_quote = poi_entries_no_quote[0]
        last_poi_no_quote = poi_entries_no_quote[-1]
        ...
        if day_num % 2 == 1 and day_num > 1 and prev_accommodation:
            ...
            if is_final_day:
                ...
                # ):
                if not (
                    prev_acc_no_quote in first_poi_no_quote and curr_acc_base_no_quote and
                    curr_acc_base_no_quote in last_poi_no_quote
                ):
        ...
        # ---------- NORMAL DAY ----------
            ...
            if curr_acc_base:
                ...
                # ):
                if not (
                    curr_acc_base_no_quote in first_poi_no_quote and
                    curr_acc_base_no_quote in last_poi_no_quote
                ):
        ...
        for meal in ["breakfast", "lunch", "dinner"]:
            if unit.get(meal) and unit[meal] != "-":
                ...
                meal_base_no_quote = meal_base.replace("'", "") ### removing quote can change later
                poi_entries_no_quote = [poi.replace("'", "") for poi in poi_entries] ### removing quote can change later
                # if not any(meal_base in poi for poi in poi_entries): ### removing quote can change later
                if not any(meal_base_no_quote in poi for poi in poi_entries_no_quote):
                    # dbg(
...
def is_valid_meal_gaps(question, tested_data):
    ...
    for i, day_plan in enumerate(tested_data[:question['days']]):
        ...
        for meal in ['breakfast', 'lunch', 'dinner']:
            ...
            # meal_base = meal_name.split(",", 1)[0].strip() ### removing quote can change later
            meal_base = meal_name.split(",", 1)[0].strip().replace("'", "")
            found = False

            for poi in poi_entries:
                # poi_name = poi.split(",", 1)[0].strip()  # ✅ exact PoI name ### removing quote can change later
                poi_name = poi.split(",", 1)[0].strip().replace("'", "")
...
def is_not_absent(question, tested_data):
    ...
    # ---------- Global checks ----------
    ok, m = is_valid_days(question, tested_data)
    if not ok:
        # dbg(m)
        return False, m

    ok, m = is_valid_visiting_city_number(question, tested_data)
    if not ok:
        # dbg(m)
        return False, m
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if unit['point_of_interest_list'] in ['', '-']:
            if day == question['days'] and is_early_departure(unit):
                continue
