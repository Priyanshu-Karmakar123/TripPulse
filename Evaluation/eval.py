        ...
        for day, dicts in days.items():
            for dct in dicts:
                ...
                for key, value in dct.items():
                    # value is expected to be (bool | None, m)
...
def eval_score(set_type: str, file_path: str):
    ...
    for idx in range(len(query_data_list)):
        ...
        for k, (ok, _) in commonsense_info.items():
            if ok is None:      
                continue
            commonsense_total += 1
            if ok:
    ...
    result["Commonsense Constraint Micro Pass Rate"] = commonsense_pass / commonsense_denominator
...
def explain_plan(query_data, tested_plan):
    ...
    for cname, (ok, m) in commonsense_info.items():
        if ok is False:
            explanations.append({
                "constraint": cname,
                "reason": m
    ...
    if (
        ...
        for cname, (ok, m) in hard_info.items():
            if ok is False:
                explanations.append({
                    "constraint": cname,
                    "reason": m
...
def explain(idx, file_path="/scratch////_golden_5day.jsonl"):
    """
    Single-line helper:
...
# def explain_batch(start_idx, end_idx, file_path="/scratch////_golden_3day.jsonl"):
#     for idx in range(start_idx, end_idx + 1):
#         print("\n" + "=" * 60)
#         explain(idx, file_path)

def explain_batch(start_idx, end_idx, file_path="/scratch////gpt5_3day_with_pc_llm_final.jsonl"):
    plans = load_line_json_data(file_path)
