...
class FinalScheduleBuilder:
    ...
    def resolve_transit_for_poi(self, poi_name: str, city: str, raw_rows: list):
        ...
        CSV_PATH = os.path.abspath(
            os.path.join(
                BASE_DIR,
                "../_database/public_transit_gtfs/all_poi_nearest_stops.csv"
            )
        )
    ...
    def _build_poi_list_for_day(self, day, day_index, days, cities):
        ...
        num_atts_today = len(attractions)
       ...
       # ===================== ATTRACTION DURATION (FINAL) =====================

        MIN_DUR = 60            # minutes (1 hour)
        MAX_DUR = 300           # minutes (6 hours)
        K_SHIFT = 16.61         # minutes (matches evaluator k = 16.61/60 hours)
        MU_D_MAX = 4            # evaluator constant (hours)

        def get_adjusted_attraction_dur(att):
            """
            Returns persona-aware, evaluator-aligned attraction duration in minutes.
            """

            # 1️⃣ Base duration from dataset (minutes)
            raw_base = attraction_duration_map.get(clean(att), ATTRACTION_DUR)

            # 2️⃣ Clamp base (safety, evaluator-consistent)
            base = max(MIN_DUR, min(raw_base, MAX_DUR))

            # 3️⃣ Persona detection
            persona = (self.persona or "").lower()
            is_adventurous = "adventure" in persona

            # 4️⃣ Max attractions in any day of the trip (stable, conservative)
            max_atts_day = max(
                len([a for a in d.get("attraction", "").split(";") if a.strip() and a.strip() != "-"])
                for d in days
            ) or 1

            # 5️⃣ Evaluator-aligned μ_d computation (minutes)
            if is_adventurous:
                # mu_d = base - k * (num_atts - 1)
                mu_d = base - (K_SHIFT * (max_atts_day - 1))
            else:
                # mu_d = base + k * (mu_d_max - num_atts)
                mu_d = base + (K_SHIFT * (MU_D_MAX - max_atts_day))

            # 6️⃣ Final clamp (hard safety)
            mu_d = max(MIN_DUR, min(mu_d, MAX_DUR))

            # 7️⃣ Return integer minutes
            return int(round(mu_d))


        def get_attraction_dur(name):
            return get_adjusted_attraction_dur(name)
    ...
    def _add_transit_for_accommodations(self, days, cities):
        """
        Post-process: Add nearest transit ONLY for stay entries.
        Safe, deterministic, no schedule impact.
        """
        ...
        for day in days:
            ...
            # Resolve correct city for this day
            if "from " in day["current_city"]:
                route = day["current_city"].replace("from ", "")
                origin, dest = route.split(" to ")
                if day["day"] == 1:
                    effective_city = dest.strip()
                elif day["day"] == len(days):
                    effective_city = origin.strip()
                else:
                    effective_city = dest.strip()
            else:
                effective_city = day["current_city"].strip()

            entries = poi_list.split(";")
            new_entries = []
            ...
            day["point_of_interest_list"] = "; ".join(new_entries)
    ...
    def build_plan_from_combined(self, reference):
        ...
        # ======================================================
        # EXTRA ATTRACTION FOR ADVENTURE SEEKER (NON-TRAVEL ONLY)
        # ======================================================
        if "adventure" in (self.persona or "").lower():

            for i, day in enumerate(days):

                # only NON-TRAVEL days
                if day["transportation"] != "-":
                    continue

                if day["attraction"] in ("", "-"):
                    continue

                # current attractions
                current = [x.strip() for x in day["attraction"].split(";") if x.strip()]

                # already 3 → stop
                if len(current) >= 3:
                    continue

                # must already have 2 (we only add ONE extra)
                if len(current) != 2:
                    continue

                city = day_city[i]

                # find ONE unused attraction
                for a in attractions_by_city.get(city, []):
                    name = a["name"]
                    if name.lower() not in used_attractions:
                        used_attractions.add(name.lower())
                        current.append(f"{name}, {city}")
                        break

                # write back
                day["attraction"] = "; ".join(current) + ";"
        ...
        self._add_transit_for_accommodations(days, cities)
